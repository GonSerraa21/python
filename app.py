...
# --- PÁGINA DE INICIO ---
if menu == "Inicio":
    st.info("Panel de Control Principal")
    
    # CÁLCULO DE ESTADÍSTICAS
    num_urgencias = len(urg.lista_espera_urg)
    # Filtramos el diccionario global para separar analíticas de consultas médicas
    num_analiticas = len([p for p in db_pacientes.values() if p.especialidad == "Analítica"])
    num_consultas = len([p for p in db_pacientes.values() if p.especialidad != "Analítica"])

    # VISUALIZACIÓN DE CONTADORES
    col1, col2, col3 = st.columns(3)
    col1.metric("🚑 Pacientes Urgencias", num_urgencias)
    col2.metric("🩺 Citas Médicas", num_consultas)
    col3.metric("🩸 Analíticas", num_analiticas)

...
# ==============================================================================
# 2. CONFIGURACIÓN DE LA MEMORIA COMPARTIDA (GLOBAL)
# ==============================================================================
...
def poner_musica():
    ...
    st.sidebar.markdown(
        f'<audio controls loop style="width: 100%;"><source src="{musica_url}" type="audio/mp3"></audio>', 
...
if menu == "Inicio":
    ...
    # Mostramos la nueva imagen corporativa
...
# --- ENTRADA CONSULTA ---
    ...
    # Usamos un formulario para que sea más limpio
    with st.form("checkin_consulta"):
        codigo_input = st.text_input("Introduzca su CÓDIGO de cita:")
        btn_verificar = st.form_submit_button("Buscar Paciente")
    
    # Si pulsamos verificar, guardamos el paciente encontrado en la 'memoria de la sesión'
    if btn_verificar:
        if codigo_input in db_pacientes:
            ...
            if paciente._especialidad != "Analítica":
                ...
                # Borramos mensajes anteriores si los hubiera
                if 'mensaje_resultado' in st.session_state:
                    del st.session_state['mensaje_resultado']
            else:
    ...
    # Fuera del formulario, comprobamos si hay un paciente cargado en memoria
    if 'paciente_actual_consulta' in st.session_state:
        ...
        st.write("Seleccione la situación actual:")
        
        col1, col2, col3 = st.columns(3)
        
        # Al pulsar estos botones, ya no importa si el botón de arriba se "reinicia"
        # porque el paciente sigue en st.session_state

...
# --- IMAGEN EN EL SIDEBAR (ARRIBA) ---
try:
    # Ajustamos el ancho a 150px para que se vea "pequeñito"
    st.sidebar.image("image_0.png", use_container_width=True)
except:
    st.sidebar.image("https://cdn-icons-png.flaticon.com/512/3304/3304567.png", width=100)
    ...
    with tab2:
        if not urg.lista_espera_urg:
            ...
            for i, p in enumerate(urg.lista_espera_urg):
                ...
                # --- CAMBIO AQUÍ ---
                # Antes: st.progress(max(0, min(100, p.prioridad)) / 100)
                # Ahora: Invertimos (100 - p.prioridad) para que el número bajo llene más la barra
                valor_invertido = 100 - p.prioridad
                
                # Nos aseguramos de que no se salga de 0 a 100 (por si la prioridad fuera negativa o >100)
                barra_final = max(0, min(100, valor_invertido))
                
                st.progress(barra_final / 100)
